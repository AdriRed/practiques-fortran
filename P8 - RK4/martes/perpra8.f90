program prepra8
   ...
   common /potential/alpha, delta, V_0
   double precision :: alpha, delta, V_0
...
subroutine wave_function_calculator(energy, xini, xend, steps, yini, result_xs, phi)
   double precision, intent(in) :: energy, xini, xend
   double precision, dimension(2), intent(in) :: yini
   integer, intent(in) :: steps

   double precision, dimension(steps), intent(out) :: result_xs, phi
   ...
   E = energy
   call rungekutta4(steps, xini, xend, 2, yini, schrodinger_equation, result_xs, temp_result_ys)
   
   normalization_constant = sum(temp_result_ys(1, 1:steps)**2)*step
   phi(1:steps) = temp_result_ys(1, 1:steps)/dsqrt(normalization_constant)
...
function schrodinger_equation(x, ys, nequs) result(dy)
   ...
   dy(1) = ys(2)
   dy(2) = 2d0/h2me*(potencial(x)-E)*ys(1)
...
double precision function potencial(x) result(retval)
   ...
   common /potential/alpha, delta, V_0
   double precision :: alpha, delta, V_0
   retval = V_0*dsinh(alpha/delta)/(dcosh(alpha/delta)+dcosh(x/delta))
end function potencial
